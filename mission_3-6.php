...
$boards = file('mission_3-6.txt',FILE_IGNORE_NEW_LINES);
$num =count($boards);
$dataFile = "mission_3-6.txt";   
$date = new DateTime();
$str = $num  . '<>' . $_GET['name'] . '<>' . $_GET['comment']
        . '<>' . $date ->format('Y-m-d-H-i-s') ."\n";
$fp = fopen('mission_3-6.txt','a');
fwrite($fp, $str);
fclose($fp);
if (!empty($password)) {
$fp2 = fopen ("mission_3-6.txt","a");
fputs ($fp2, $writepas."\n");
}
}
      ...
      $password2 = $_POST['password2'];
       ...
       for($r = 0; $r < count($pasCon) ; $r++) {  
           ...
           if ($delData[0] == $delete && $pasData[0] == $password2){
               ...
               file_put_contents("mission_3-6.txt", $delCon);
...
if (isset($_POST['edit']))
    {   $number3=$_POST['name3'];
       $password3=$_POST['password3'];
        $file_edit = file("mission_3-6.txt");
...
if (isset($_GET['message']) && isset($_GET['hidden1'])) {
        $contents = file("mission_3-6.txt");
  $fp1 = fopen('mission_3-6.txt','w');
